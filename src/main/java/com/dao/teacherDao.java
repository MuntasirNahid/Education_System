...
public class teacherDao {
	...
	public int countCourseEnrollment() {
		int i = 0;
		try {
			String sql = "select * from course_enrollment";
			PreparedStatement ps = conn.prepareStatement(sql);
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				i++;
	...
	public int countCourseEnrollmentByTeacherId(int tid) {
		int i = 0;
		try {
	...
	public boolean checkOldPassword(int teacherId, String oldPassword) {
		boolean f = false;
	...
	public boolean changePassword(int teacherId, String newPassword) {
		boolean f = false;
	...
	public boolean editTeacherProfile(teacher d) {
		boolean f = false;

		try {
			String sql = "update teacher set fullName=?,dob=?,qualification=?,course_teaches=?,email=?,mobNo=? where id=?";
			PreparedStatement ps = conn.prepareStatement(sql);
